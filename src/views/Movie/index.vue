...
<style scoped>
	#content{ flex:1; overflow:auto; margin-bottom: 50px; height: 100%; position: relative; display: flex; flex-direction:column;}
...
export default {
    ...
    mounted(){
        ...
        setTimeout(()=>{
            this.axios.get('/api/api/getLocation').then((res)=>{
                var msg = res.data.msg;
                if(msg === 'ok'){

                    var nm = res.data.data.nm;
                    var id = res.data.data.id;
                    if( this.$store.state.city.id == id ){return;} //如果当前定位城市=选定的城市，不弹窗定位城市
                    messageBox({
                        title : '定位',
                        content : nm,
                        cancel : '取消',
